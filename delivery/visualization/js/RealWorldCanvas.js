...
var RealWorldVisualization = function(height, width, gridWidth, gridHeight, foregroundContext, backgroundContext) {
  ...
  /*
   * Draw everything that is related to the background (image, obstacles, ...) [Layer 0 and below]
   */
   ...
   * Draw the obstacles
   */
  this.drawObstacles = function(layer) {
    for (var i in layer) {
      var p = layer[i];
      ...
      if (p.Type == "Obstacle") {
        this.drawObstacle(p.x, p.y, p.Color);
      }
    }
  }

  /*
   * Draw everything that is related to the foreground (BaseStation, Uav, ...) [Layer 1 and above]
   */
  this.drawForeground = function(layers) {
    // Only draw layers without obstacles (= layers[0])
    for (layer in layers) {
      if (layer != 0) {
        this.drawLayer(layers[layer]);
      }
    }
  };
  ...
  this.drawLayer = function(portrayalLayer) {
    for (var i in portrayalLayer) {
      ...
      if (p.Type == "BaseStation") {
        this.drawBaseStation(p.x, p.y, p.Color);
      }
    }
  };
  ...
  /*
   * Draw an Obstacle in the specified grid cell
   * x, y: Grid co-ordinates
   * color: color of the Obstacle
   */
  this.drawObstacle = function(x, y, color) {
    ...
    // Draw rectangle
    backgroundContext.fillRect(x0, y0, dx, dy);
  };
